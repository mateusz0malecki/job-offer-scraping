...
class OffersPagination(BaseModel):
    page_number: int = None
    page_size: int = None
    total_record_count: int = None
    pagination: dict = {}

    def __init__(self, instances, first, last, page, page_size, *args, **kwargs):
        ...
        if last >= len(instances):
            ...
            self.pagination["next"] = f"/job-offers/?page={page + 1}&page_size={page_size}"
        if page > 1:
            self.pagination["previous"] = f"/job-offers/?page={page - 1}&page_size={page_size}"
        else:
